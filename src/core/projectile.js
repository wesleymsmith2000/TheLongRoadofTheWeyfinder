export function createProjectile(x, y, vx, vy, options = {}) {
  const projectile = {
    ...
    maxRicochets: Math.max(0, Math.floor(options.maxRicochets ?? 0)),
    ricochetCount: Math.max(0, Math.floor(options.ricochetCount ?? 0)),
    ...
    guidedTargeting: Boolean(options.guidedTargeting),
    targetingReticleKey: options.targetingReticleKey ?? null,
    sourceOffset: options.sourceOffset ?? null,
    sourceZ: options.sourceZ ?? 0,
    endZ: options.endZ ?? 0,
    widthEnvelopeScale: options.widthEnvelopeScale ?? 1,
    sweepBeam: Boolean(options.sweepBeam),
    sweepStart: options.sweepStart ?? null,
    sweepTarget: options.sweepTarget ?? null,
