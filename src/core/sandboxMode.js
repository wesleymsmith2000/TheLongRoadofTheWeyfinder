    ...
    spawns: [
      {
        ...
        ...(typeof options.speed === 'number' ? { speed: positiveNumber(options.speed, 22) } : {}),
      },
    ],
...
export function sandboxDefinitionFromLevel(level, options = {}) {
  ...
  for (const wave of level.waves ?? []) {
    for (const [index, spawn] of (wave.spawn ?? []).entries()) {
      spawns.push({
        ...
        atDistance: wave.atDistance,
        count: spawn.count,
        interval: (spawn.spacing ?? 0) / roadSpeed,
        distanceInterval: spawn.spacing ?? 0,
  ...
  for (const obstacle of level.obstacles ?? []) {
    ...
    spawns.push({
      ...
      atDistance: obstacle.atDistance,
      count: 1,
...
function normalizeSpawn(spawn, index = 0) {
  ...
  return {
    ...
    atDistance: positiveNumber(spawn.atDistance, null),
    ...
    distanceInterval: positiveNumber(spawn.distanceInterval, 0),
