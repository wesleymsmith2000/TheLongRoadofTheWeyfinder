...
export function fireSecondary(game) {
  ...
  game.playerProjectiles.push(
    createProjectile(muzzle.x, muzzle.y, launch.vx, launch.vy, {
      ...
      blastPierceCells: def.blastPierceCells ?? 0,
...
function upgradedSecondaryDefinition(game, weapon) {
  ...
  if (weapon === 'cannon') {
    return scaleProjectileVisuals({
      ...
      blastDamage: base.blastDamage * 2 * multiplier(game, 'cannonBlastDamage'),
      blastRadius: CELL_SIZE * 5.1 * multiplier(game, 'cannonBlastRadius'),
      blastPierceCells: base.blastPierceCells ?? 1.5,
  ...
  if (weapon === 'rocket') {
    return scaleProjectileVisuals({
      ...
      blastDamage: base.blastDamage * multiplier(game, 'rocketBlastDamage'),
      ...
      acceleration: (base.maxSpeed * multiplier(game, 'rocketMaxVelocity')) / Math.max(0.001, base.lifetime * 0.25),
      explodeOnExpire: true,
    }, visualScale);
  }
  if (weapon === 'beam') {
